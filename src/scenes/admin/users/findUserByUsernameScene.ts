...
export class FindUserByUsernameScene extends Scene implements SceneView {
    create(): BaseScene<BotContextMessageUpdate> {
        this.scene.enter(async (ctx) => {
            return ctx.reply('Введите логин NlogN: ')
        })
        ...
        this.scene.on('text', async (ctx) => {
            ...
            if (students.length === 0) {
                await ctx.reply('Пользователей с таким логином не найдено.')
